...
namespace LevelnetAdjustment.utils {
  public class ExceHelperl {
    ...
    /// <returns>观测手簿路径</returns>
    /// <exception cref="Exception"></exception>
    internal static string ExportHandbook(List<RawData> rds, string path, List<InputFile> files) {
      IWorkbook workbook;
      ...
      try {
        ...
        var rand = Commom.GenerateRandomString(8);
        var pathCopy = Path.Combine(Path.GetDirectoryName(path), Path.GetFileNameWithoutExtension(path) + rand + Path.GetExtension(path));
        using (FileStream fs = new FileStream(pathCopy, FileMode.Create, FileAccess.Write)) {
          workbook.Write(fs);
        };
        return pathCopy;
      }
